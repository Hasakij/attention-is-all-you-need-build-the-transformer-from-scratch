...
def init_decoder_layer_parameters(d_model, num_heads, d_ff):
    ...
    w_q_self = torch.randn(d_model, d_model) * 0.02
    w_k_self = torch.randn(d_model, d_model) * 0.02
    w_v_self = torch.randn(d_model, d_model) * 0.02
    w_o_self = torch.randn(d_model, d_model) * 0.02
    
    # cross-attention projections
    w_q_cross = torch.randn(d_model, d_model) * 0.02
    w_k_cross = torch.randn(d_model, d_model) * 0.02
    w_v_cross = torch.randn(d_model, d_model) * 0.02
    w_o_cross = torch.randn(d_model, d_model) * 0.02
    ...
    # 3 layernorms
    self_gamma = torch.ones(d_model)
    self_beta = torch.zeros(d_model)
    cross_gamma = torch.ones(d_model)
    cross_beta = torch.zeros(d_model)
    ffn_gamma = torch.ones(d_model)
    ffn_beta = torch.zeros(d_model)
...
def init_embedding_and_projection_parameters(vocab_size, d_model, tie_weights=True):
    ...
    src_embedding = (torch.randn(vocab_size, d_model) * 0.02).requires_grad_(True)
    tgt_embedding = (torch.randn(vocab_size, d_model) * 0.02).requires_grad_(True)

    if tie_weights:
        output_projection = tgt_embedding
    else:
        output_projection = (torch.randn(vocab_size, d_model) * 0.02).requires_grad_(True)
...
def shift_targets_right_with_start_token(target_ids, start_token_id):
    ...
    return torch.cat([start_column, target_ids[:, :-1]], dim=1)
...
def zero_pad_column_and_pad_token_rows(smoothed_distribution, gold_token_ids, pad_id):
    ...
    out[:, :, pad_id] = 0 # zero the pad column

    pad_rows = (gold_token_ids.to(torch.long) == pad_id) # zero rows where gold_id == pad_id
    out[pad_rows] = 0
    return out
...
def average_loss_over_non_pad_tokens(total_loss, gold_token_ids, pad_id):
    ...
    n_non_pad = torch.sum(mask)
    n_safe = torch.clamp(n_non_pad, 1)
    return total_loss / n_safe
...
def compute_token_accuracy_ignoring_pad(log_probabilities, gold_token_ids, pad_id):
    ...
    predicted = torch.argmax(log_probabilities, dim=-1) # (B, T)
    matches = (predicted == gold_token_ids)
    non_pad = (gold_token_ids != pad_id)

    numerator = torch.sum(matches & non_pad)
    denominator = torch.sum(non_pad)
    denominator_safe = torch.clamp(denominator, min=1)
    acc = numerator.float() / denominator_safe.float()
    return acc
...
def initialize_adam_optimizer_state(parameter_list):
    ...
    for param in parameter_list:
        m_i = torch.zeros_like(param, requires_grad=False)
        v_i = torch.zeros_like(param, requires_grad=False)
        m.append(m_i)
        v.append(v_i)
    return{
        'm': m,
...
def update_adam_second_moment(v_prev, grad, beta2):
    ...
    with torch.no_grad():
        v_t = beta2 * v_prev + (1 - beta2) * torch.square(grad)
    return v_t
...
def apply_adam_step_to_all_parameters(parameter_list, optimizer_state, learning_rate, beta1=0.9, beta2=0.98, epsilon=1e-9):
    ...
    with torch.no_grad():
        for i, param in enumerate(parameter_list):
            ...
            m_t = update_adam_first_moment(m_list[i], grad, beta1)
            v_t = update_adam_second_moment(v_list[i], grad, beta2)
...
def run_training_loop_for_steps(batches, parameter_list, model_params, optimizer_state, num_steps, config):
    ...
    for step_number in range(1, num_steps + 1):
        batch_idx = (step_number - 1) % len(batches)
        src_batch, tgt_batch = batches[batch_idx]

        loss = run_training_step_with_backprop(
            src_batch, tgt_batch,
            parameter_list, model_params, optimizer_state,
            step_number, config
        )
        losses.append(loss)
    return losses
